...
dataINLA <- function(dat, level='reef') {
    ...
    if (level=='reef') {
        dat.1 <- rbind(dat %>%
                   dplyr:::select(Cover,Tows,Year,P_CODE.mod, P_CODE.mod1, REEF_NAME, REEF_NAME1, REEF_NAME2, YEAR, YEAR1, W, Cvr1, Cvr0),
                   newdata.hcc) %>%
        as.data.frame
    } else {
        dat.1 <- rbind(dat %>%
...
#############################################################################                                                                                                                                                 
## The following function prepares a data.frame for output as a javascript ##                                                                                                                                                 
## variable definition with JSON                                           ##                                                                                                                                                 
##   df:    the dataframe                                                  ##                                                                                                                                                 
##   name:  the name of the variable to assign to the json object          ##                                                                                                                                                 
#############################################################################                                                                                                                                                 
send_df_to_js <- function(df, name){
    cat(
        paste(
            'var ',name,' = ',jsonlite::toJSON(df),';'
          , sep="")
    )
}
